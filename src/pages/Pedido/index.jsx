...
function Pedido() {
  ...
  const mostrarPedidoId = async (e) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_BASE_URL}/pedidos/${id}`, {
        method: "GET",
        headers: {
  ...
  async function handleCancelarPedido(e) {
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_BASE_URL}/pedidos/cancelado/${id}`, {
        method: "PUT",
        headers: {
          "content-type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    async function handleFinalizarPedido(e) {
      try {
        const response = await fetch(`${import.meta.env.VITE_APP_BASE_URL}/pedidos/finalizado/${id}`, {
          method: "PUT",
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  return (
    <main className="tela-pedido">
      ...
      <div className="botoes">
        <button className="btn green" onClick={handleFinalizarPedido}> Concluir </button>
        <button className="btn red" onClick={handleCancelarPedido}> Cancelar </button>
      </div>
